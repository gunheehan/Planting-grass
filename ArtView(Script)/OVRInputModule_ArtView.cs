...
namespace UnityEngine.EventSystems
{
    ...
    /// </summary>
    public class OVRInputModule_ArtView : PointerInputModule
    {
        ...
        [Header("Distance")]
        public bool isArtView = true;
        public float hoverDistance = 0.12f;
        public float clickDistance = 0.08f;
        ...
        protected OVRInputModule_ArtView()
        {}
...
#region PointerEventData pool
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            float distance = 0;
            RaycastResult[] temp = null;

            if(isArtView)
            {
                distance = Vector3.Distance(raycast.worldPosition, rayTransform.position);

                if (distance > hoverDistance) return null;

                temp = new RaycastResult[m_RaycastResultCache.Count];
                m_RaycastResultCache.CopyTo(temp);
            }
            ...
            if(isArtView)
            {
                ...
                {
                    foreach(RaycastResult o in temp)
                    {
                        ButtonEvent e = o.gameObject.GetComponent<ButtonEvent>();
                        if(e!=null)
                        {
                            // interactable 의 true 입력한 이벤트 동작
                            if(e.GetComponent<Button>().interactable)
                            {
                                StartCoroutine(e.inputModuleOnClickEvent());
                                e.GetComponent<Button>().onClick.Invoke();
                                AudioManager.instance.PlayClickSound();
                            }
                            return null;
                        }
                    }
                    m_MouseState.SetButtonState(PointerEventData.InputButton.Left, PointerEventData.FramePressState.Pressed, leftData);
